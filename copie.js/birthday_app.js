...
// 					<li><p><b>${person.lastName}</b> ${person.firstName} will turn ${futureAge} years old on the ${new Date(person.birthday).toLocaleString("en-US", { month: "long" })}<time datetime="${fullDate}"> ${new Date(person.birthday).toLocaleString("en-US", { day: "numeric" })}<sup>${nthDate(dateNow)}</sup></time></p></li>
// 					<li><time datetime="${fullDate}">${fullDate}</time></li>
// 					<li>${dayLeft < 0 ? dayLeft * -1 + " " + "days ago" : dayLeft + " days"}</li>
// 					<li>
