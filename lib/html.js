    ...
    return people.map(person => {
        ...
        <article data-id="${person.id}" value="${person.id}" class="article">
            <ul>
                ...
                <li><b>${person.lastName}</b> ${person.firstName}<br>will turn ${futureAge} years old on the <time datetime="${fullDate}"> ${new Date(person.birthday).toLocaleString("en-Us", { day: "numeric" })}<sup>${nthDate(date)}</sup></time> of ${new Date(person.birthday).toLocaleString("en-US", { month: "long" })}</p></li>
