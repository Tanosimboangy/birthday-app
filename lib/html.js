        ...
        <article data-id="${person.id}" value="${person.id}" class="article">
            <ul>
                ...
                <li>
                    ...
                    <span>
                        Turns ${futureAge} on ${new Date(person.birthday).toLocaleString("en-US", { month: "long" })} <time datetime="${fullDate}"> ${new Date(person.birthday).toLocaleString("en-Us", { day: "numeric" })}${nthDate(date)}</time>
